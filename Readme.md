...
CDBUS IP Core
=======================================
(This is the 8-bit version; for the 32-bit version, please switch to the corresponding branch.)
...
* Introduces an arbitration mechanism, similar to the CAN bus, to automatically avoid conflicts.
* Supports dual baud rates for high-speed communication;
  the maximum baud rate of the high-speed phase is `sysclk` ÷ 3 (e.g. 50 Mbps for 150 MHz `sysclk`).
* Supports unicast, multicast and broadcast.
* The maximum user data size is 253 bytes.
* Hardware packing, unpacking, verification and filtering, reducing CPU load and development effort.
* Compatible with traditional RS-485 hardware (the arbitration function remains effective).
...
Tips:
 - When a high-priority node needs to send unimportant data, its transmission permit time (TX_PERMIT_LEN) can be dynamically increased.
...
The RX data sampling point is at 1/2 bit; for TX readback, it's at 3/4 bit.
...
In CDBUS-A mode, if the low-speed portion takes too long, it can become a bottleneck for communication efficiency.
...
| FILTER_MSK1       |  0x1d   | RD/WR  | 0xff            | Multicast mask1                                      |
...
Not used in arbitration mode, nor for the break character automatically generated in BS mode.
...
For single-rate operation, set DIV_HS to the same value as DIV_LS.
...
## Simulation
Install `iverilog` (>= v10) and `cocotb`, go to the `tests/` directory, and run `./test_all.sh` or `./test_all.sh test_xxx.py`.
(You can view the waveform `cdbus.vcd` with GTKWave.)


## Ready-to-Use Devices

The CDCTL controller family incorporates the CDBUS IP Core, providing peripheral interfaces such as SPI, I<sup>2</sup>C, and PCIe.  
...
Other related projects that may interest you:
