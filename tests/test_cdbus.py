...
async def csr_read(dut, address, burst = False):
    await RisingEdge(dut.clk)
...
async def test_cdbus(dut):
    ...
    value = await csr_read(dut, REG_VERSION, True)
    ...
    await csr_write(dut, REG_DIV_LS_H, 0, True)
    await csr_write(dut, REG_DIV_LS_L, 39, True) # 1Mbps
    await csr_write(dut, REG_DIV_HS_H, 0, True)
    await csr_write(dut, REG_DIV_HS_L, 3, True)  # 10Mbps
    await csr_write(dut, REG_FILTER, 0x00, True) # set local filter to 0x00
    # TODO: reset rx...

    await csr_write(dut, REG_TX, 0x01, True) # disguise as node 0x01 to send data
    await csr_write(dut, REG_TX, 0x00, True)
    await csr_write(dut, REG_TX, 0x01, True)
    await csr_write(dut, REG_TX, 0xcd, True)
    ...
    await csr_write(dut, REG_TX, 0x0f, True) # disguise as node 0x0f to send data
    await csr_write(dut, REG_TX, 0x00, True)
    await csr_write(dut, REG_TX, 0x01, True)
    await csr_write(dut, REG_TX, 0xcd, True)
    await csr_write(dut, REG_TX_CTRL, BIT_TX_START | BIT_TX_RST_POINTER)
